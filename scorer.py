...
def read_file(file, file_dict={}):
    ...
    with open(file, encoding='utf-8') as f:
        for line_num, line in enumerate(f):
            ...
            if line_num == 0:
                if parts == ["article_id", "entity_mention", "start_offset", "end_offset", "main_role",
                             "fine_grained_roles"]:
            ...
            file_dict[key] = (main_role, ast.literal_eval(fine_grained_roles))
    ...
    for key in file_dict:
        print("key: ",key)
        print("file_dict: ", file_dict[key])

    return file_dict

def read_file_old(file,file_dict={}):
    file_dict = {}
    with open(file, encoding='utf-8') as f:
        for line_num, line in enumerate(f):
            parts = line.strip().split('\t')
            ...
            # check if header exists and skip if it does
            if line_num == 0:
                if parts == ["article_id", "entity_mention", "start_offset", "end_offset", "main_role", "fine_grained_roles_1", "fine_grained_roles_2"]:
                    logger.info(f"Skipping header in file: {file}")
                    continue
            ...
            end_offset = parts[3]
            main_role = parts[4]
            fine_grained_roles_1 = parts[5]
            try:
                fine_grained_roles_2 = parts[6]
            except:
                fine_grained_roles_2 = ""
            key = (article_id, entity_mention, start_offset, end_offset)
            ...
            if fine_grained_roles_2 == "NaN":
                file_dict[key] = (main_role, [fine_grained_roles_1])
            else:
                file_dict[key] = (main_role, [fine_grained_roles_1, fine_grained_roles_2])
            logger.debug(f"Processed key: {key} with value: {file_dict[key]}")
...
def main(gold_file_path, pred_file_path):
    ...
    return emr, micro_precision, micro_recall, micro_f1, main_role_accuracy
