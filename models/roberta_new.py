...
class RoBERTa(nn.Module):
    def __init__(self, hyperparamters ,num_labels=22):
        ...
        self.tokenizer = RobertaTokenizer.from_pretrained("roberta-base")  # TODO: test differente models here as well
    ...
    def predict(self, trainer, model,tokenizer, all_classes, dev, threshold_list, result_save_name="predictions_with_labels.csv"):
        """
        ...
        for true_class in true_classes:
            ...
            for i in range(len(indices)):
                temp.append(all_classes[indices[i]])
            tc_1.append(temp)

        results_df["fine_grained_roles_ground_truth"] = tc_1
        ...
        fine_grained_roles_predictions = []
        for index, line in results_df.iterrows():
            ...
            fine_grained_roles_predictions.append([class_prediction])

        results_df["fine_grained_roles_predictions"] = fine_grained_roles_predictions
        ...
        # predicted main-role
        for name in results_df["fine_grained_roles_predictions"]:
            roles.append(sub_role_to_main_role(name[0]))
        ...
        def convert_df_to_right_format(test_df):
            ...
            for item in test_df["fine_grained_roles"]:
                temp2 = []
                for entry in item:
                    #print("entry: ", entry)
                    if entry != None:
                ...
                temp.append(temp2)
                print("temp2: ", temp2)
            formated_df["fine_grained_roles"] = temp
        ...
        for threshold in threshold_list:
                ...
                for j in range(1, len(sorted_outputs)):
                    ...
                    if sorted_outputs[0] - second_highest_value > threshold:
                        ...
                        break
                            ...
                            results_df["fine_grained_roles_predictions"][index][0]):
                        second_highest_classes[-1] = "NaN"
            ...
            for index, line in results_df.iterrows():
                ...
                sorted_indices = np.argsort(output)[:: -1]
                sorted_outputs = np.array(output)[sorted_indices]
                third_highest_classes.append("NaN")
                for j in range(index_third_class, len(sorted_outputs)):
                    third_highest_value = sorted_outputs[j]
                    third_highest_class = sorted_indices[j]

                    if j == index_third_class:
                        third_highest_values.append(third_highest_value)
                        third_highest_classes.append(all_classes[third_highest_class])
                    else:
                        third_highest_values[-1] = third_highest_value
                        third_highest_classes[-1] = third_highest_class

                    if sorted_outputs[0] - third_highest_value > threshold:
                        third_highest_classes[-1] = "NaN"
                        break

                    elif sub_role_to_main_role(third_highest_classes[-1]) != sub_role_to_main_role(
                            results_df["fine_grained_roles_predictions"][index][0]):
                        third_highest_classes[-1] = "NaN"
            ...
            for index, line in results_df.iterrows():
                if second_highest_classes[index] != "NaN" and second_highest_classes[index] != None:
                    print("second_highest_classes[index]: ", second_highest_classes[index])
                    results_df["fine_grained_roles_predictions"][index].append(second_highest_classes[index])
                if third_highest_classes[index] != "NaN" and third_highest_classes[index] != None:
                    results_df["fine_grained_roles_predictions"][index].append(third_highest_classes[index])
                    print("third_highest_classes[index]: ", third_highest_classes[index])

            formated_df["main_role"] = results_df["main_role_prediction"]
            formated_df["fine_grained_roles"] = results_df["fine_grained_roles_predictions"]
...
def train(model, df,df_dev, model_save_name, result_save_name, thredhold_list,dynamic_seed):
    ...
    #df["labels"] = model.label_encoder.fit_transform(df["fine_grained_roles"])
    all_classes = ['Guardian', 'Martyr', 'Peacemaker', 'Rebel', 'Underdog', 'Virtuous', 'Instigator', 'Conspirator', 'Tyrant', 'Foreign Adversary', 'Traitor', 'Spy', 'Saboteur', 'Corrupt', 'Incompetent', 'Terrorist', 'Deceiver', 'Bigot','Forgotten', 'Exploited', 'Victim', 'Scapegoat']

    def convert_labels_to_multilabel_format(fine_grained_roles, num_labels=22):

        # Erstelle einen Tensor mit der Größe (1, num_labels), initialisiert mit Nullen
        multilabel_tensor = torch.zeros(num_labels)

        for fine_grained_role in fine_grained_roles:
        # Setze die entsprechenden Positionen auf 1
            if fine_grained_role in all_classes:
                multilabel_tensor[all_classes.index(fine_grained_role)] = 1
    ...
    dataset = dataset.map(
        lambda x: {
            'labels': convert_labels_to_multilabel_format(x['fine_grained_roles'])},
        batched=False
    )

    #dataset["labels"] = model.label_encoder.fit_transform(df["fine_grained_roles"])
    ...
    model.predict(trainer,model.model, model.tokenizer,all_classes, 0, thredhold_list, result_save_name=result_save_name)
